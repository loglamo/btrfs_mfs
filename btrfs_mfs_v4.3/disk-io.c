...
// dat: modify this fucntion because change superblock location
int btrfs_validate_super_partition(struct btrfs_fs_info *fs_info,
			 struct btrfs_super_block *sb, int mirror_num, int partition_order)
{
	u64 nodesize = btrfs_super_nodesize(sb);
	u64 sectorsize = btrfs_super_sectorsize(sb);
	int ret = 0;
...
"superblock metadata_uuid doesn't match metadata uuid of fs_devices: %pU != %pU",
	...
	// dat: modify here
	u64 sb_offset = BTRFS_SUPER_INFO_SIZE * partition_order;
	if (mirror_num >= 0 &&
	    btrfs_super_bytenr(sb) != btrfs_sb_offset_partition(mirror_num, sb_offset)) {
		btrfs_err(fs_info, "super offset mismatch %llu != %u",
			  btrfs_super_bytenr(sb), BTRFS_SUPER_INFO_OFFSET);
		ret = -EINVAL;
	}
...
// dat: modify this function because change superblock location
static int btrfs_validate_mount_super_partition(struct btrfs_fs_info *fs_info, int partition_order)
{
	return btrfs_validate_super_partition(fs_info, fs_info->super_copy, 0, partition_order);
}
		      ...
		      char *options)
{
	...
	// dat: error here, let see what is the error
...
// dat: this function call from btrfs_fill_super(), add partition_order to the function
int __cold open_ctree_partition(struct super_block *sb, struct btrfs_fs_devices *fs_devices,
		      char *options, int partition_order)
{
	u32 sectorsize;
	u32 nodesize;
	u32 stripesize;
	 ...
	 * Read super block and check the signature bytes only
	 */
	disk_super = btrfs_read_dev_super_partition(fs_devices->latest_dev->bdev, partition_order);
	if (IS_ERR(disk_super)) {
		ret = PTR_ERR(disk_super);
		goto fail_alloc;
	}
	...
	memcpy(fs_info->super_for_commit, fs_info->super_copy,
	       sizeof(*fs_info->super_for_commit));
	// dat: error here, let see what is the error
	ret = btrfs_validate_mount_super_partition(fs_info, partition_order);
	if (ret) {
		btrfs_err(fs_info, "superblock contains fatal errors");
		ret = -EINVAL;
		goto fail_alloc;
	...
	ret = btrfs_read_chunk_tree_partition(fs_info, partition_order);
	if (ret) {
		btrfs_err(fs_info, "failed to read chunk tree: %d", ret);
		goto fail_tree_roots;
	}
...
ALLOW_ERROR_INJECTION(open_ctree_partition, ERRNO);
...
// dat: this function is called from btrfs_read_dev_super_partition()
struct btrfs_super_block *btrfs_read_dev_one_super_partition(struct block_device *bdev,
						   int copy_num, bool drop_cache, int partition_order)
{
	struct btrfs_super_block *super;
	struct page *page;
	u64 bytenr, bytenr_orig;
	...
	int ret;
	u64 sb_offset = BTRFS_SUPER_INFO_SIZE * partition_order;
	bytenr_orig = btrfs_sb_offset_partition(copy_num, sb_offset);
	// printk(KERN_ERR "btrfs_read_dev_one_super_partition: bytenr_orig = %llu\n", bytenr_orig);
	ret = btrfs_sb_log_location_bdev_partition(bdev, copy_num, READ, &bytenr, sb_offset);
	// printk(KERN_ERR "btrfs_read_dev_one_super_partition: bytenr = %llu\n", bytenr);
	// printk(KERN_ERR "btrfs_read_dev_one_super_partition: ret = %llu\n", ret);
	if (ret == -ENOENT)
		return ERR_PTR(-EINVAL);
	else if (ret)
		return ERR_PTR(ret);
...
// dat: this fucntion is called from btrfs_get_bdev_and_sb(), open_ctree()
struct btrfs_super_block *btrfs_read_dev_super_partition(struct block_device *bdev, int partition_order)
{
	struct btrfs_super_block *super, *latest = NULL;
	int i;
	u64 transid = 0;
	 ...
	 */
	for (i = 0; i < 1; i++) {
		super = btrfs_read_dev_one_super_partition(bdev, i, false, partition_order);
		if (IS_ERR(super))
			continue;
	...
	if (IS_ERR(super))
		printk(KERN_ERR "btrfs_read_dev_super_partition: super is error\n");
	return super;
}
 ...
 */
// dat: modify this function to write superblock to partition
			    ...
			    struct btrfs_super_block *sb, int max_mirrors)
{
	...
	shash->tfm = fs_info->csum_shash;
	// dat: find the superblock bytenr first
	u64 sb_bytenr_from_fs_info = fs_info->super_copy->bytenr;
	// printk(KERN_INFO "write_dev_supers: sb_bytenr_from_fs_info = %llu\n", sb_bytenr_from_fs_info);
	u64 sb_offset = 0;
	if (sb_bytenr_from_fs_info < BTRFS_SUPER_INFO_OFFSET_1)
		sb_offset = sb_bytenr_from_fs_info - BTRFS_SUPER_INFO_OFFSET;
	else if (sb_bytenr_from_fs_info < BTRFS_SUPER_INFO_OFFSET_2)
		sb_offset = sb_bytenr_from_fs_info - BTRFS_SUPER_INFO_OFFSET_1;
	else
		sb_offset = sb_bytenr_from_fs_info - BTRFS_SUPER_INFO_OFFSET_2;
	// printk(KERN_INFO "write_dev_supers: sb_offset = %llu\n", sb_offset);
	for (i = 0; i < max_mirrors; i++) {
		...
		bytenr_orig = btrfs_sb_offset_partition(i, sb_offset);
		ret = btrfs_sb_log_location_partition(device, i, WRITE, &bytenr, sb_offset);
		if (ret == -ENOENT) {
			continue;
		} else if (ret < 0) {
...
static int wait_dev_supers(struct btrfs_device *device, int max_mirrors)
{
	...
	struct btrfs_fs_info *fs_info = device->fs_info;
	u64 sb_bytenr_from_fs_info = fs_info->super_copy->bytenr;
	// printk(KERN_INFO "wait_dev_supers: sb_bytenr_from_fs_info = %llu\n", sb_bytenr_from_fs_info);
	u64 sb_offset = 0;
	if (sb_bytenr_from_fs_info < BTRFS_SUPER_INFO_OFFSET_1)
		sb_offset = sb_bytenr_from_fs_info - BTRFS_SUPER_INFO_OFFSET;
	else if (sb_bytenr_from_fs_info < BTRFS_SUPER_INFO_OFFSET_2)
		...
		sb_offset = sb_bytenr_from_fs_info - BTRFS_SUPER_INFO_OFFSET_2;
	// printk(KERN_INFO "wait_dev_supers: sb_offset = %llu\n", sb_offset);
