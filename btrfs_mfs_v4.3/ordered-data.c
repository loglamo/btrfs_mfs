...
static struct btrfs_ordered_extent *alloc_ordered_extent(
			struct btrfs_inode *binode, u64 file_offset, u64 num_bytes,
			...
			u64 offset, unsigned long flags, int compress_type)
{
	...
	if (flags &
		...
		ret = btrfs_qgroup_free_data(binode, NULL, file_offset, num_bytes, &qgroup_rsv);
		if (ret < 0)
			return ERR_PTR(ret);
	} else {
		/*
		 ...
		 */
		ret = btrfs_qgroup_release_data(binode, file_offset, num_bytes, &qgroup_rsv);
		if (ret < 0)
			return ERR_PTR(ret);
	}
   ...
   //mfs: Convert local bytenr to global bytenr
    struct inode *VFSinode = &binode->vfs_inode; 
    struct btrfs_fs_info *fs_info = btrfs_sb(VFSinode->i_sb);
    struct btrfs_super_block *sb = fs_info->super_copy;
    u64 total_bytes = sb->total_bytes;
    u64 nodesize = sb->nodesize;
    //disk_bytenr = total_bytes*fs_info->id + disk_bytenr;
	...
	entry->inode = igrab(&binode->vfs_inode);
	 ...
	 */
	spin_lock(&binode->lock);
	btrfs_mod_outstanding_extents(binode, 1);
	spin_unlock(&binode->lock);
