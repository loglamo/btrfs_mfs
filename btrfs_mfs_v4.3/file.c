...
{   
    ...
    //printk(KERN_INFO "btrfs_buffered_write: writing to inode %llu of fs_info %d\n", inode->i_ino, fs_info->id);
	ret = btrfs_inode_lock(BTRFS_I(inode), ilock_flags);
	if (ret < 0)
		return ret;

    //mfs: Update mapping if in need
    /*
    struct address_space *f_mapping = file->f_mapping;
...
again:
        ...
        //printk(KERN_INFO "btrfs_buffered_write: inode type is %d\n", inode->has_remote);
		/*
...
out:
    ...
    //printk(KERN_INFO "btrfs_buffered_write: Unlock inode to complete writing\n");
...
int btrfs_release_file(struct inode *inode, struct file *filp)
{
    ...
    struct inode *f_inode = file_inode(filp);
	...
	*/
    if (test_and_clear_bit(BTRFS_INODE_FLUSH_ON_CLOSE,
			       &BTRFS_I(f_inode)->runtime_flags))
			filemap_flush(f_inode->i_mapping);
	return 0;
}
...
static int btrfs_file_mmap(struct file	*filp, struct vm_area_struct *vma)
{
    ...
    //if (f_inode->has_remote == 0) //This is a remote inode
...
static loff_t btrfs_file_llseek(struct file *file, loff_t offset, int whence)
{
    ...
    struct inode *f_inode;
    //mfs: Check remote inode; at here inode is at remote fs
    /*
    if (inode->has_remote == 1) { 
        ...
        f_inode = inode;
    struct btrfs_fs_info *inode_fs_info = btrfs_sb(inode->i_sb);
    */
    //struct btrfs_fs_info *f_inode_fs_info = btrfs_sb(inode->i_sb);
    //printk(KERN_INFO "btrfs_file_llseek: Called to seek in inode %llu of fs_info %d...\n", inode->i_ino, inode->fs_id);
    //if (f_inode)
    //    printk(KERN_INFO "btrfs_file_llseek: Called to seek in f_inode %llu of fs_info %d...\n", f_inode->i_ino, f_inode->fs_id);
	...
	case SEEK_HOLE:
		...
		//btrfs_inode_lock(BTRFS_I(f_inode), BTRFS_ILOCK_SHARED);
...
static int btrfs_file_open(struct inode *inode, struct file *filp)
{
    ...
    //mfs: Checking
    /*
    printk(KERN_INFO "btrfs_file_open: inode with fs_id %d\n", inode->fs_id);
    struct inode *f_inode = filp->f_inode;
    printk(KERN_INFO "btrfs_file_open: f_inode with fs_id %d, and associated fs_id %d\n", f_inode->fs_id, f_inode->associated_fs_id);
    struct path *f_path = &filp->f_path;
    struct dentry *p_dentry = f_path->dentry;
    printk(KERN_INFO "btrfs_file_open: f_path with dentry of fs_id %d with associated fs_id %d \n", p_dentry->fs_id, p_dentry->associated_fs_id);
    */

   /* 
   if (p_dentry->associated_path) {
      f_path = p_dentry->associated_path; 
      printk(KERN_INFO "btrfs_file_open: Did update f_path with associate path\n");
   }
    */


    /*
    if ((inode->associated_inode) && (inode->fs_id != 0)) {
        f_inode = inode->associated_inode;
        printk(KERN_INFO "This inode has connected to a local inode\n");
    } else {
        f_inode = inode;
    }
    */
    ...
    //mfs: Update dentry of f_path
   /* 
    if (f_path->dentry) {
        f_dentry = f_path->dentry;
        //f_inode = f_dentry->d_inode;
        printk(KERN_INFO "File dentry with remote dentry fs_id is %d\n", f_dentry->fs_id);
        if ((f_dentry->associated_dentry) && (f_dentry->remote == 0)) {// this is local dentry
            f_path  = f_dentry->associated_path; //Path of remote dentry
            //filp->f_inode = inode;
            printk(KERN_INFO "btrfs_file_open: Update f_path with remote dentry and f_inode of file\n");
        }
    }
    */
    ...
    /*
    if ((inode->associated_inode) && (inode->fs_id != 0)) {
        inode = inode->associated_inode;
        printk(KERN_INFO "btrfs_file_open: Did update inode when openning\n");
    }
    */
	...
	if (ret) {
        //printk(KERN_INFO "btrfs_file_open: Error at fsverify_file_open\n");
		return ret;
    }
    //printk(KERN_INFO "btrfs_file_open:  Enter generic_file_open\n");
...
static ssize_t btrfs_file_read_iter(struct kiocb *iocb, struct iov_iter *to)
{
    ...
    //mfs: Check whether file has its remote inode
    struct file *file = iocb->ki_filp;
    struct address_space *mapping = file->f_mapping;
    struct inode *f_inode = file->f_inode;
    int remote = f_inode->has_remote;

   /* 
    if (remote == 1)
        printk(KERN_INFO "btrfs_file_read_iter: This is a local inode\n");
    else if (remote == 2)
        printk(KERN_INFO "btrfs_file_read_iter: This is a normal inode keeping file\n");
    else
        printk(KERN_INFO "btrfs_file_read_iter: This is a remote inode keeping file\n");
    */

    //printk(KERN_INFO "btrfs_file_read_iter: f_inode id from iocb is %llu \n", f_inode->i_ino);
    

    /*    
    if (f_inode->has_remote == 0) {
        struct inode *local_inode = f_inode->associated_inode;
        file->f_inode = local_inode;
        file->f_mapping = local_inode->i_mapping;
        printk(KERN_INFO "btrfs_file_read_iter: Did update reading path to the local \n");
     }
    */

    //struct inode *f_inode_tmp = file->f_inode;
    //struct inode *f_inode = file_inode(file);

    //mfs: Update f_mapping
    //mapping = f_inode->i_mapping;
    //f_inode_tmp = f_inode;
	...
	if (iocb->ki_flags & IOCB_DIRECT) {
		...
		if (ret < 0 || !iov_iter_count(to) ||
		    ...
		    //iocb->ki_pos >= i_size_read(f_inode))
			return ret;
	}
    //printk(KERN_INFO "btrfs_file_read_iter: Start to read from pagecache...\n");

	return btrfs_filemap_read(iocb, to, ret);
}
