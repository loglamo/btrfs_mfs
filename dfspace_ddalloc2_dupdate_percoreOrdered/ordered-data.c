 ...
 */
// La: Add new item to percore-list
void insert_pcpu_entry(struct list_head *entry, struct list_head *pcpu_list) {
    struct list_head *current_list;
    preempt_disable();
    current_list = this_cpu_ptr(pcpu_list);
    list_add_tail(entry, current_list);
//    if (!list_empty(current_list)) {
//        printk("New entry has already added...\n");
//     }
    preempt_enable();
}
...
*/
// La: For per-core ordered_extent list
   insert_pcpu_entry(&entry->root_extent_list, root->pcpu_ordered_extents);
   
   atomic_inc(&root->nr_pcpu_ordered_extents); 
   if (atomic_read(&root->nr_pcpu_ordered_extents) == 1) {
       spin_lock(&fs_info->ordered_root_lock);
       BUG_ON(!list_empty(&root->ordered_root));
       list_add_tail(&root->ordered_root, &fs_info->ordered_roots);
       spin_unlock(&fs_info->ordered_root_lock);
 ...
 */
KTDEF(mutex_lock_ordered_extent_mutex);
KTDEF(spin_lock_ordered_extent_lock);
KTDEF(init_work_btrfs_run_ordered_extent_work);
			       ...
			       const u64 range_start, const u64 range_len)
{
    ...
    ktget(&localclock[1]);
    ktput(localclock, mutex_lock_ordered_extent_mutex);
#endif
	...
	spin_lock(&root->ordered_extent_lock);
    ktget(&localclock[1]);
    ktput(localclock, spin_lock_ordered_extent_lock);
#endif 
    struct list_head *pcpu_list = root->pcpu_ordered_extents;
    preempt_disable();
    struct list_head *current_list = this_cpu_ptr(pcpu_list);
	//list_splice_init(&root->ordered_extents, &splice);
	list_splice_init(current_list, &splice);
	while (!list_empty(&splice) && nr) {
        ...
        ktget(&localclock[1]);
        ktput(localclock, init_work_btrfs_run_ordered_extent_work);
	...
	list_splice_tail(&skipped, current_list);
	//list_splice_tail(&splice, &root->ordered_extents);
	list_splice_tail(&splice, current_list);
	//spin_unlock(&root->ordered_extent_lock);
    preempt_enable();
...
KTDEF(mutex_lock_ordered_operations);
KTDEF(spin_lock_ordered_root_lock);
KTDEF(btrfs_wait_ordered_extents);
...
//	INIT_LIST_HEAD(&splice);
    ...
    ktget(&localclock[1]);
    ktput(mutex_lock_ordered_operations);
#endif

    ktget(&localclock[0]);
	spin_lock(&fs_info->ordered_root_lock);
    ktget(&localclock[1]);
    ktput(localclock, spin_lock_ordered_root_lock);
...
/*
	...
	while (!list_empty(&splice) && nr) {
        ...
        ktget(&localclock[1]);
        ktput(localclock, btrfs_wait_ordered_extents);
		btrfs_put_root(root);
...
//while (nr) {
    root = list_first_entry_or_null(&fs_info->ordered_roots, struct btrfs_root, ordered_root);
    if (!root) {
        spin_unlock(&fs_info->ordered_root_lock);
        return;
    }
    ...
    //struct list_head *current_list = root->pcpu_ordered_extents;
    ktget(&localclock[0]);
