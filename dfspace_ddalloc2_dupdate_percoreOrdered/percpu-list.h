...
// La: Initialize per-cpu list_head from LA, initilize a per-cpu list head struct defined global
/*
int init_pcpu_list_head_v2(struct list_head *pcpu_list) {
    int cpu;
    for_each_possible_cpu(cpu) {
        struct list_head *head_ptr = per_cpu_ptr(pcpu_list, cpu);
        INIT_LIST_HEAD(head_ptr);
    }
    printk("Did initialization of ppcu_ordered_extents lists\n");
    return 0;
}
int init_pcpu_list_head_v3(struct list_head **ppcpu_list_head) {
    struct list_head *pcpu_list_head = alloc_percpu(struct list_head);
    int cpu;
    ...
    for_each_possible_cpu(cpu) {
        struct list_head *head = per_cpu_ptr(pcpu_list_head, cpu);
        INIT_LIST_HEAD(head);
    }
    *ppcpu_list_head = pcpu_list_head;
    printk("Did initialization of per-core ordered extents\n");
    return 0;
}
*/
