				...
				struct btrfs_inode *inode)
{
  ...
  */
     ret =  btrfs_update_inode_item(trans, root, inode);
     return ret;
}
...
KTDEF(start_delalloc_inodes_delalloc_mutex);
KTDEF(start_delalloc_inode_filemap_fdatawrite_wbc);
				 ...
				 bool in_reclaim_context)
{
    ...
    ktget(&localclock[1]);
    ktput(localclock, start_delalloc_inodes_delalloc_mutex);
#endif
	spin_lock(&root->delalloc_lock);
    ...
    /*
		if (in_reclaim_context &&
			...
			continue;
    */
      binode = list_first_entry_or_null(&root->delalloc_inodes, struct btrfs_inode,
             delalloc_inodes);
      if (!binode) {
          spin_unlock(&root->delalloc_lock);
          goto out;
	  ...
	  if (!inode) {
	   ...
	   list_add_tail(&work->list, &works);
            ...
            ktget(&localclock[1]);
            ktput(localclock, start_delalloc_inode_filemap_fdatawrite_wbc);
...
out:
    ...
    */
    ...
    if (binode) {
        spin_lock(&root->delalloc_lock);
        list_move_tail(&binode->delalloc_inodes, &root->delalloc_inodes);
        spin_unlock(&root->delalloc_lock);
    }
	return ret;
}
...
KTDEF(mutex_lock_delalloc_root_mutex);
KTDEF(start_delalloc_inodes);
			       ...
			       bool in_reclaim_context)
{
    ...
    ktget(&localclock[1]);
    ktput(localclock, mutex_lock_delalloc_root_mutex);
#endif
    // La: Replace the mutext with spin_lock
    //spin_lock(&fs_info->delalloc_root_lock_test);
	spin_lock(&fs_info->delalloc_root_lock);
	...
	while (!list_empty(&splice)) {
		...
		list_move_tail(&root->delalloc_root,
			       &fs_info->delalloc_roots);
		spin_unlock(&fs_info->delalloc_root_lock);
        //La: Show addr of the root
      //  printk(KERN_INFO "Currently iterated delalloc root is: %px\n", root);

        ktget(&localclock[0]);
		ret = start_delalloc_inodes(root, &wbc, false, in_reclaim_context);
        ktget(&localclock[1]);
        ktput(localclock, start_delalloc_inodes);
...
out:
    ...
    //spin_unlock(&fs_info->delalloc_root_lock_test);
	return ret;
}
