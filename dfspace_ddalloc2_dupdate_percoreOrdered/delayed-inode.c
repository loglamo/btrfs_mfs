		...
		struct btrfs_inode *btrfs_inode)
{
	...
	struct btrfs_root *root = btrfs_inode->root;
    /*
    struct btrfs_fs_info *fs_info = root->fs_info; // La adds this line
    ...
    struct rb_root_cached *mapping_tree = &fs_info->mapping_tree;
    struct rb_root_cached *block_group_cache_tree = &fs_info->mapping_tree;
    */
	u64 ino = btrfs_ino(btrfs_inode);
    //printk(KERN_INFO "Object id is %llu", ino);
    //printk(KERN_INFO "Address of root is %px", root);
	int ret;
				...
				int mod)
{
    ...
    // La: Add below line
    count = delayed_root->nodes;
    //printk(KERN_INFO "Current number of nodes in the list is: %d\n", count);
				     ...
				     struct btrfs_delayed_item *first_item)
{
	...
	if (batch.nr == 1) {
		...
		first_key.offset = curr->index;
		batch.keys = &first_key;
		//batch.data_sizes = curr->data_len;
