			 ...
			 u64 objectid)
{
	...
	INIT_LIST_HEAD(&root->ordered_extents); //La: Original version
    // La: Per-core version
    init_pcpu_list_head_v3(&root->pcpu_ordered_extents);
...
void btrfs_init_fs_info(struct btrfs_fs_info *fs_info)
{
    ...
    // La: Add the below tested lock initilization
    spin_lock_init(&fs_info->delalloc_root_lock_test);
...
*/
// La: For per-core ordered extents of this root 
    ...
    for_each_possible_cpu(cpu) {
        ...
        if (!list_empty(current_list)) {
            list_for_each_entry(ordered, current_list, root_extent_list)
                set_bit(BTRFS_ORDERED_IOERR, &ordered->flags);
        }
    }
}
