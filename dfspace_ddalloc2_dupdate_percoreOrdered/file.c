...
KTDEF(btrfs_delalloc_reserve_metadata);
KTDEF(balance_dirty_pages_ratelimited_flags);
KTDEF(prepare_pages);
					       ...
					       struct iov_iter *i)
{
	...
	while (iov_iter_count(i) > 0) {
        ...
        ktget(&localclock[1]);
        ktput(localclock, btrfs_delalloc_reserve_metadata);
...
again:  
        ...
        ktget(&localclock[1]);
        ktput(localclock, balance_dirty_pages_ratelimited_flags);
		if (ret) {
        ...
        ktget(&localclock[1]);
        ktput(localclock, prepare_pages);
			    ...
			    const struct btrfs_ioctl_encoded_io_args *encoded)
{
	...
	if (encoded) {
        ...
        ktget(&localclock[1]);
        ktput(localclock, btrfs_buffered_write);
		num_sync = num_written;
	}
...
static ssize_t btrfs_file_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
    ...
    ktget(&localclock[0]);
    ret =  btrfs_do_write_iter(iocb, from, NULL);
    ktget(&localclock[1]);
    ktput(localclock, btrfs_do_write_iter);
    return ret;
}
