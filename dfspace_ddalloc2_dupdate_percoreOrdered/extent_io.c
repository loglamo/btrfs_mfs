 ...
 */
KTDEF(folio_lock);
KTDEF(folio_wait_writeback);
KTDEF(__extent_writepage);
	...
	while (!done && !nr_to_write_done && (index <= end) &&
		...
		for (i = 0; i < nr_folios; i++) {
			...
			if (!folio_trylock(folio)) {
                ...
                ktget(&localclock[0]);
				//folio_lock(folio);
                sbtrfs_folio_lock(folio);
                ktget(&localclock[1]);
                ktput(localclock, folio_lock);
			}
			...
			if (wbc->sync_mode != WB_SYNC_NONE) {
                ...
                ktget(&localclock[1]);
                ktput(localclock, folio_wait_writeback);
			}
            ...
            ktget(&localclock[1]);
            ktput(localclock, __extent_writepage);
			if (ret < 0) {
				done = 1;
				break;
			}
...
KTDEF(btrfs_zoned_data_reloc_lock);
KTDEF(extent_write_cache_pages);
KTDEF(extent_writepages_submit_write_bio);
		      ...
		      struct writeback_control *wbc)
{
    ...
    ktget(&localclock[1]);
    ktput(localclock, btrfs_zoned_data_reloc_lock);
    ktget(&localclock[0]);
	ret = extent_write_cache_pages(mapping, &bio_ctrl);
    ktget(&localclock[1]);
    ktput(localclock, extent_write_cache_pages);
    ktget(&localclock[0]);
	submit_write_bio(&bio_ctrl, ret);
    ktget(&localclock[1]);
    ktput(localclock, extent_writepages_submit_write_bio);
