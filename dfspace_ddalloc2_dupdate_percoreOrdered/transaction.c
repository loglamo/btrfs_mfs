...
KTDEF(btrfs_run_delayed_refs_commit);
KTDEF(btrfs_create_pending_block_groups_first);
KTDEF(btrfs_start_dirty_block_groups);
KTDEF(btrfs_end_transaction_cur_trans);
KTDEF(wait_for_commit_cur_trans);
KTDEF(wait_for_commit_prev_trans);
KTDEF(btrfs_start_delalloc_flush_cur_trans);
KTDEF(btrfs_wait_delalloc_flush);
int btrfs_commit_transaction(struct btrfs_trans_handle *trans)
{
	...
	if (!test_and_set_bit(BTRFS_DELAYED_REFS_FLUSHING,
        ...
        ktget(&localclock[1]);
        ktput(localclock, btrfs_run_delayed_refs_commit);
		if (ret)
			goto lockdep_trans_commit_start_release; }
	...
	btrfs_create_pending_block_groups(trans);
    ktget(&localclock[1]);
    ktput(localclock, btrfs_create_pending_block_groups_first);
	...
	if (!test_bit(BTRFS_TRANS_DIRTY_BG_RUN, &cur_trans->flags)) {
		...
		if (run_it) {
            ...
            ktget(&localclock[1]);
            ktput(localclock, btrfs_start_dirty_block_groups);
	...
	if (cur_trans->state >= TRANS_STATE_COMMIT_START) {
        ...
        ktget(&localclock[1]);
        ktput(localclock, btrfs_end_transaction_cur_trans);

        ktget(&localclock[0]);
		wait_for_commit(cur_trans, want_state);
        ktget(&localclock[1]);
        ktput(localclock, wait_for_commit_cur_trans);
	...
	if (cur_trans->list.prev != &fs_info->trans_list) {
		...
		if (prev_trans->state < want_state) {
            ...
            ktget(&localclock[1]);
            ktput(localclock, wait_for_commit_prev_trans);
	...
	ktget(&localclock[1]);
    ktput(localclock, btrfs_start_delalloc_flush_cur_trans);
    ...
    ktget(&localclock[1]);
    ktput(localclock, btrfs_wait_delalloc_flush);
