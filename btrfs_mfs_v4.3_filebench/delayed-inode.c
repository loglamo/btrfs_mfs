...
//mfs: Modify this function, this inode is dir inode
int btrfs_inode_delayed_dir_index_count(struct btrfs_inode *inode)
{
    //mfs: Check current fs_info
    struct inode *cur_inode = &inode->vfs_inode;
    struct btrfs_fs_info *cur_fs_info = btrfs_sb(cur_inode->i_sb);
    //printk(KERN_INFO "btrfs_inode_delayed_dir_index_count: Get dir_index count from fs_info %d\n", cur_fs_info->id);
	struct inode *local_inode;
    struct btrfs_inode *b_local_inode;

    b_local_inode = inode;

    if ((cur_inode->associated_inode) && (cur_inode->has_remote == 0)) {
        //printk(KERN_INFO "btrfs_inode_delayed_dir_index:Get local inode of this remote inode\n");
        local_inode = cur_inode->associated_inode;
        b_local_inode = BTRFS_I(local_inode);
    } else {
        b_local_inode = inode;
    }

    //struct btrfs_delayed_node *delayed_node = btrfs_get_delayed_node(inode);
    struct btrfs_delayed_node *delayed_node = btrfs_get_delayed_node(b_local_inode);
    


	if (!delayed_node) {
        //printk(KERN_INFO "btrfs_inode_delayed_dir_index_count: Not get delayed node\n");
		return -ENOENT;
    }
	/*
	...
	b_local_inode->index_cnt = delayed_node->index_cnt;
	btrfs_release_delayed_node(delayed_node);
	return 0;
}
