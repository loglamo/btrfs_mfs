...
extern struct btrfs_fs_info *global_fs_info_list[BTRFS_SUPER_INFO_COUNT];
atomic_t global_create_count = ATOMIC_INIT(0);
				   ...
				   bool keep_locked, bool no_inline)
{
	...
	while (num_bytes > 0) {
		...
		ram_size = ins.offset;
        //mfs: For global objectid block start
			    ...
			    u32 pgoff, u8 *csum, const u8 * const csum_expected)
{
	...
	if (memcmp(csum, csum_expected, fs_info->csum_size)) {
		//printk(KERN_INFO "data_csum: check_sector gets wrong with csum: %s and expected: %s\n", csum, csum_expected);
		return -EIO;
    }
	return 0;
}
			...
			u32 bio_offset, struct bio_vec *bv)
{
	...
	if (btrfs_check_sector_csum(fs_info, bv->bv_page, bv->bv_offset, csum,
				    csum_expected)) {
		//printk(KERN_INFO "data_csum: check_sector gets wrong\n");
        goto zeroit;
    }
				...
				struct btrfs_rename_ctx *rename_ctx)
{
    ...
    struct btrfs_inode *local_inode; //Try to unlink for local inode
    struct inode *v_inode = &inode->vfs_inode;
    struct btrfs_fs_info *fs_info_inode = btrfs_sb(v_inode->i_sb);
    if ((v_inode->associated_inode) && (v_inode->has_remote == 0)) {// This is a remote inode
        //printk(KERN_INFO "btrfs_unlink_inode: Try to unlink a local inode of a remote inode\n");
        local_inode = BTRFS_I(v_inode->associated_inode); //Get the btrfs_inode of the local inode
    } else {
        local_inode = inode;
    }
	u64 ino = btrfs_ino(inode);
    ...
    //printk(KERN_INFO "btrfs_unlink_inode: Called with fs_info_of dir id = %d... and fs_info_id of inode = %d\n", fs_info->id, fs_info_inode->id);
	 ...
	 */
	//if (inode->dir_index) {
	if (local_inode->dir_index) {
		ret = btrfs_delayed_delete_inode_ref(local_inode);
		if (!ret) {
			index = local_inode->dir_index;
			goto skip_backref;
		}
	}
...
skip_backref:
	...
	if (!rename_ctx) {
		...
		btrfs_del_inode_ref_in_log(trans, root, name, local_inode, dir_ino);
	...
	btrfs_run_delayed_iput(fs_info, local_inode);
err:
	btrfs_free_path(path);
	if (ret)
		goto out;
	...
	inode_inc_iversion(&local_inode->vfs_inode);
	inode_inc_iversion(&dir->vfs_inode);
	//inode_set_ctime_current(&inode->vfs_inode);
	inode_set_ctime_current(&local_inode->vfs_inode);
...
static int btrfs_unlink(struct inode *dir, struct dentry *dentry)
{
    ...
    //mfs: Check and get local inode
    
    
    struct inode *local_inode;
    if ((inode->associated_inode) && (inode->has_remote == 0))
        local_inode = inode->associated_inode;
    if (local_inode)
        inode = local_inode;
     

    //mfs: Check remote inode
    struct btrfs_fs_info *fs_info = btrfs_sb(inode->i_sb);
    int fs_id = fs_info->id;

    printk(KERN_INFO "btrfs_unlink: Inode of fs %d, dentry->inode of fs %d", dir->fs_id, inode->fs_id);
	int ret;
	...
	btrfs_record_unlink_dir(trans, BTRFS_I(dir), BTRFS_I(inode),
				false);

    //ret = btrfs_unlink_inode(trans, BTRFS_I(dir), BTRFS_I(d_inode(dentry)), &fname.disk_name);
	ret = btrfs_unlink_inode(trans, BTRFS_I(dir), BTRFS_I(inode),
				 &fname.disk_name);
...
static int btrfs_setattr(struct mnt_idmap *idmap, struct dentry *dentry,
			 struct iattr *attr)
{
    printk(KERN_INFO "btrfs_setattr: Called...\n");
	struct inode *inode = d_inode(dentry);
	struct btrfs_root *root = BTRFS_I(inode)->root;
	int err;
			       ...
			       struct btrfs_key *location, u8 *type)
{
    ...
    //mfs
    struct inode *dir_inode = &dir->vfs_inode;
    struct btrfs_fs_info *fs_info = btrfs_sb(dir_inode->i_sb);
    //printk(KERN_INFO "btrfs_lookup_dentry() via inode_by_name: at fs_info %d with dentry from fs_id %d \n", fs_info->id, dentry->fs_id);
    //printk(KERN_INFO "btrfs_inode_by_name: dentry has associated_fs_id %d\n", dentry->associated_fs_id);
...
struct inode *btrfs_lookup_dentry_mfs(struct inode *dir, struct dentry *dentry)
{  
    //printk(KERN_INFO "btrfs_lookup_dentry_mfs: Called...\n");
    struct inode *remote_dir;
    /*
    if (dentry->associated_dentry) {
        printk(KERN_INFO "btrfs_lookup_dentry_mfs: This dentry has its remote dentry...\n");
        struct dentry *associated_dentry = dentry->associated_dentry;
    }
     */
    /*
    struct inode *orig_dir;
    if (dir->has_local_dir == 1) {
        orig_dir = dir->associated_dir_inode;
        //printk(KERN_INFO "btrfs_lookup_dentry_mfs: Got original dir\n");
    } else {
        orig_dir = dir;
        //printk(KERN_INFO "btrfs_lookup_dentry_mfs: Got current dir\n");
    }
    */
    struct inode *orig_dir = dir;
	struct btrfs_fs_info *fs_info = btrfs_sb(orig_dir->i_sb);
    //printk(KERN_INFO "fs_info with id %d\n", fs_info->id);

	struct inode *inode;
	struct btrfs_root *root = BTRFS_I(orig_dir)->root;
	struct btrfs_root *sub_root = root;
	struct btrfs_key location;
	u8 di_type = 0;
	int ret = 0;

    struct inode *mfs_dir = dir;
    struct dentry *mfs_dentry;

    mfs_dentry = dentry;

    /*
    int remote = dentry->remote;
    if (dentry->associated_dentry) {
        mfs_dentry = dentry->associated_dentry;
        //printk(KERN_INFO "btrfs_lookup_dentry_mfs: This dentry has its local dentry...\n");
    } else {
        mfs_dentry = dentry;
        //printk(KERN_INFO "btrfs_lookup_dentry_mfs: This dentry is local dentry...\n");
    }
    */

    //printk(KERN_INFO "btrfs_lookup_dentry_mfs: This dentry has remote = %d\n", remote);
    //printk(KERN_INFO "btrfs_lookup_dentry_mfs: This inode has has_remote = %d\n", dir->has_remote);
    //struct inode *checked_inode = dentry->d_inode;
    //printk(KERN_INFO "btrfs_lookup_dentry_mfs: This inode of this dentry has_remote = %d\n", checked_inode->has_remote);

    /*
    if (dentry->associated_dentry) {
        printk(KERN_INFO "btrfs_lookup_dentry_mfs: This dentry has its remote dentry...\n");
        struct dentry *associated_dentry = dentry->associated_dentry;
        struct super_block *mfs_sb = associated_dentry->d_sb;
        struct dentry *mfs_root = mfs_sb->s_root;
        struct inode *mfs_inode = mfs_root->d_inode;
        fs_info = btrfs_sb(mfs_sb);
        root = BTRFS_I(mfs_inode)->root;
        sub_root = root;
        mfs_dir = mfs_inode;
        mfs_dentry = associated_dentry;
    }
    */
	...
	ret = btrfs_inode_by_name(BTRFS_I(orig_dir), mfs_dentry, &location, &di_type);
	if (ret < 0) {
        //printk(KERN_INFO "btrfs_lookup_dentry_mfs: Not pass btrfs_inode_by_name\n");
		return ERR_PTR(ret);
    }
    printk(KERN_INFO "btrfs_lookup_dentry_mfs: Pass btrfs_inode_by_name\n");

	if (location.type == BTRFS_INODE_ITEM_KEY) {
		inode = btrfs_iget(orig_dir->i_sb, location.objectid, root);
		if (IS_ERR(inode))
			return inode;
...
"inode mode mismatch with dir: inode mode=0%o btrfs type=%u dir type=%u",
	...
	ret = fixup_tree_root_location(fs_info, BTRFS_I(orig_dir), mfs_dentry,
				       &location, &sub_root);
	if (ret < 0) {
		if (ret != -ENOENT) {
           // printk(KERN_INFO "btrfs_lookup_dentry_mfs: Not pass fixup_tree_root_location\n");
			inode = ERR_PTR(ret);
        }
		else
			inode = new_simple_dir(orig_dir, &location, root);
	} else {
		inode = btrfs_iget(orig_dir->i_sb, location.objectid, sub_root);
		btrfs_put_root(sub_root);
...
//mfs: For assigning fs_info to create inode with round-robin
struct btrfs_fs_info *btrfs_fs_placement_round_robin(struct inode *orig_dir, int count_index) {
    struct btrfs_fs_info *fs_info = btrfs_sb(orig_dir->i_sb);
    int local_id = fs_info->id;
    struct btrfs_fs_info *assigned_fs_info;
  
    int rr_index = count_index % BTRFS_SUPER_INFO_COUNT; 
    
    assigned_fs_info = global_fs_info_list[rr_index];
    //printk(KERN_INFO "Assigned fs_info with id %d \n", assigned_fs_info->id);
    return assigned_fs_info;
}


//mfs: For assigning fs_info to create inode with selecting current CPU
struct btrfs_fs_info *btrfs_fs_placement(struct inode *orig_dir) {
    struct btrfs_fs_info *fs_info = btrfs_sb(orig_dir->i_sb);
    int local_id = fs_info->id;
    struct btrfs_fs_info *assigned_fs_info;
    ...
    if (cpu_id != local_id)
        assigned_fs_info = global_fs_info_list[cpu_id];
    else
        assigned_fs_info = fs_info;
   
    
    //assigned_fs_info = global_fs_info_list[32];
    //printk(KERN_INFO "Assigned fs_info with id %d \n", assigned_fs_info->id);
    return assigned_fs_info;
}

//mfs: Get inode of the filesystem
struct inode *btrfs_map_mfsdir(struct btrfs_fs_info *fs_info) {
    struct super_block *sb = fs_info->sb;
    struct dentry *mfs_root = sb->s_root;
    struct inode *mfs_inode = mfs_root->d_inode;

    return mfs_inode;
}

//mfs: Get Fs/ device id from the inode
int btrfs_get_device_id(struct inode *cur_inode) {
    int ret;
    struct btrfs_fs_info *fs_info = btrfs_sb(cur_inode->i_sb);
    ret = fs_info->id;
...
static struct dentry *btrfs_lookup(struct inode *dir, struct dentry *dentry,
				   unsigned int flags)
{   
    printk(KERN_INFO "btrfs_lookup: Called...\n");
...
//mfs: Lookup in mfs
static struct dentry *btrfs_lookup_mfs(struct inode *dir, struct dentry *dentry,
				   unsigned int flags)
{   
    printk(KERN_INFO "btrfs_lookup_mfs: Called with dir inode from fs_id %d and dentry from fs_id %d...\n", dir->fs_id, dentry->fs_id);
	struct inode *inode = btrfs_lookup_dentry_mfs(dir, dentry);
    //mfs: Get associated dentry and mfs inode if they exist
   
   /*
    struct inode *mfs_inode;
    struct dentry *associated_dentry;
    struct inode *associated_inode;

    if ((dentry->fs_id == 0) && (dentry->associated_dentry)) {
        int associated_fs_id = dentry->associated_fs_id;
        associated_dentry = dentry->associated_dentry;
        struct inode *d_inode = associated_dentry->d_inode;
        struct btrfs_fs_info *associated_fs_info = btrfs_sb(d_inode->i_sb);
        mfs_inode = btrfs_map_mfsdir(associated_fs_info);
        printk(KERN_INFO "btrfs_lookup_mfs: This dir from fs_id %d\n", associated_fs_info->id);
        associated_inode = btrfs_lookup_dentry_mfs(mfs_inode, associated_dentry);

        if (associated_inode == ERR_PTR(-ENOENT)) {
            associated_inode = NULL;
        } else {
            printk(KERN_INFO "btrfs_lookup: Did lookup associated inode in remote FS\n");
        }

        d_splice_alias(associated_inode, associated_dentry);
    }
    */
...
static int btrfs_get_dir_last_index(struct btrfs_inode *dir, u64 *index)
{
	...
	if (dir->index_cnt == (u64)-1) {
		...
		if (ret) {
			...
			if (ret) {
                printk(KERN_INFO "btrfs_get_dir_last_index: Done path\n");
				goto out;
            }
		}
	}
...
static int btrfs_opendir(struct inode *inode, struct file *file)
{
    ...
    /* 
    if (inode->has_remote == 1) {
        f_inode = inode->associated_inode;
        printk(KERN_INFO "btrfs_opendir: This is a local inode");
     }
    */
    printk(KERN_INFO "btrfs_opendir: called and got f_inode from file...\n");


    /*
    if (inode->has_remote == 0) {
        f_inode = inode->associated_inode;
        printk(KERN_INFO "btrfs_opendir: Get a local inode for file accessing");
     }
     */
	//ret = btrfs_get_dir_last_index(BTRFS_I(inode), &last_index);
	ret = btrfs_get_dir_last_index(BTRFS_I(f_inode), &last_index);
	if (ret)
		return ret;
...
static int btrfs_opendir_mfs(struct inode *inode, struct file *file)
{
	struct btrfs_file_private *private;
	u64 last_index;
	int ret;
    ...
    */
     ...
     */
    int is_local_dir = inode->is_local_dir;
    printk(KERN_INFO "btrfs_opendir_mfs: Called open this inode with local_dir = %d\n", is_local_dir);
...
{   
    printk(KERN_INFO "btrfs_dir_llseek: Called...\n");
...
{   
    printk(KERN_INFO "btrfs_real_readdir: Called...\n");
...
int btrfs_set_inode_index_mfs(struct btrfs_inode *dir, struct btrfs_inode *local_dir, u64 *index, u64 *local_index)
{
	int ret = 0;
    //printk(KERN_INFO "btrfs_set_inode_index: Called...\n");

	if (local_dir->index_cnt == (u64)-1) {
		ret = btrfs_inode_delayed_dir_index_count(local_dir);
		if (ret) {
            //printk(KERN_INFO "btrfs_set_inode_index_mfs: Set inode index\n");
			ret = btrfs_set_inode_index_count(dir);
			if (ret)
				return ret;
		}
	...
	*index = dir->index_cnt;
	*local_index = local_dir->index_cnt;
	local_dir->index_cnt++;
	dir->index_cnt++;
			   ...
			   struct btrfs_new_inode_args *args)
{
    ...
    //printk(KERN_INFO "btrfs_create_newinode: Called...\n");
	path = btrfs_alloc_path();
	if (!path)
		return -ENOMEM;
...
int btrfs_create_new_inode_mfs(struct btrfs_trans_handle *trans,
			   struct btrfs_new_inode_args *args)
{
	struct inode *dir = args->dir;
    struct inode *local_dir = args->local_dir;
	struct inode *inode = args->inode;
    struct inode *local_inode = args->local_inode;
	...
	if (args->orphan) {
		...
		ret = btrfs_set_inode_index_mfs(BTRFS_I(dir), BTRFS_I(local_dir), &BTRFS_I(inode)->dir_index, &BTRFS_I(local_inode)->dir_index);
		if (ret)
			goto out;
	}
	/* index_cnt is ignored for everything but a dir. */
			       ...
			       struct inode *inode)
{
    ...
    //printk(KERN_INFO "btrfs_create_common: Called...\n");
...
//mfs
static int btrfs_create_common_mfs(struct inode *dir, struct inode *local_dir, struct dentry *dentry,
			       struct inode *inode, struct inode *local_inode)
{
	struct btrfs_fs_info *fs_info = btrfs_sb(dir->i_sb);
	struct btrfs_root *root = BTRFS_I(dir)->root;
	struct btrfs_new_inode_args new_inode_args = {
	...
	err = btrfs_create_new_inode_mfs(trans, &new_inode_args);
	if (!err)
		d_instantiate_new(dentry, inode);
...
{
	struct inode *inode;
    printk(KERN_INFO "btrfs_mknod: Called...\n");
...
static int btrfs_create_mfs(struct mnt_idmap *idmap, struct inode *orig_dir,
			struct dentry *dentry, umode_t mode, bool excl)
{
	struct inode *inode, *mfs_dir, *dir;
    struct dentry *remote_dentry;
    struct path remote_path;
    struct mnt_idmap *remote_idmap;
    int ret;
    int local_device, remote_device;

    //if (dentry->called_from_FS == 1)
    //    goto create_inode;

    //mfs: Get name
    struct qstr *dname = &dentry->d_name;
    unsigned char *name = dname->name;
    //printk(KERN_INFO "btrfs_create_mfs: Called... \n");

    if (dentry->remote == 1) {
        //printk(KERN_INFO "btrfs_create_mfs: return at once, because recursive\n");
        return 0;
    }

    if (dentry->associated_dentry) {
        //printk(KERN_INFO "btrfs_create_mfs: this is local dentry and its remote dentry was create, can skip this step \n");
        return 0;
    }

    //mfs: Assign partition/FS
    atomic_inc(&global_create_count);
    int count_index = atomic_read(&global_create_count);

    struct btrfs_fs_info *assigned_fs = btrfs_fs_placement_round_robin(orig_dir, count_index); 
    //struct btrfs_fs_info *assigned_fs = btrfs_fs_placement(orig_dir); 
    mfs_dir = btrfs_map_mfsdir(assigned_fs);
    local_device = btrfs_get_device_id(orig_dir);
    remote_device = assigned_fs->id;

    //mfs: Add new shadow or remote dentry and new inode
    if (remote_device == local_device) { // Case: Assigned FS is the local FS
        inode = new_inode(orig_dir->i_sb);
	    if (!inode)
		    return -ENOMEM;
	    inode_init_owner(idmap, inode, orig_dir, mode);
	    inode->i_fop = &btrfs_file_operations;
	    inode->i_op = &btrfs_file_inode_operations;
	    inode->i_mapping->a_ops = &btrfs_aops;
	    ret =  btrfs_create_common(orig_dir, dentry, inode);
        inode->has_remote = 2;
        //printk(KERN_INFO "btrfs_create_mfs: Normal inode are created\n");
    } else { // Case: Assigned FS is a remote FS
        //remote_dentry = kern_path_create_mfs(AT_FDCWD, name, &remote_path, 0, 1);
        remote_dentry = kern_path_create(AT_FDCWD, name, &remote_path, 0);
        if (IS_ERR(remote_dentry))
            ret = PTR_ERR(remote_dentry);
        remote_dentry->remote = 1;
        remote_dentry->d_name = dentry->d_name;
        remote_idmap = mnt_idmap(remote_path.mnt);
        remote_dentry->called_from_FS = 1;
        inode = new_inode(mfs_dir->i_sb);
	    if (!inode)
		    return -ENOMEM;
        mfs_dir->has_local_dir = 1;
        mfs_dir->associated_dir_inode = orig_dir;
	    inode_init_owner(remote_idmap, inode, mfs_dir, mode);
	    inode->i_fop = &btrfs_file_operations;
	    inode->i_op = &btrfs_file_inode_operations;
	    inode->i_mapping->a_ops = &btrfs_aops;
        inode->fs_id = remote_device;
        
        
        struct inode *local_inode = new_inode(orig_dir->i_sb);
	    if (!local_inode)
		    return -ENOMEM;
	    inode_init_owner(idmap, local_inode, orig_dir, mode);
	    local_inode->i_fop = &btrfs_file_operations;
	    local_inode->i_op = &btrfs_file_inode_operations;
	    local_inode->i_mapping->a_ops = &btrfs_aops;
        local_inode->associated_fs_id = remote_device;
	    ret =  btrfs_create_common(orig_dir, dentry, local_inode);
        local_inode->has_remote = 1; // Indicate that this is a local inode
        local_inode->associated_inode = inode;
        inode->associated_inode = local_inode;
        dentry->remote = 0;
        inode->has_remote = 0; // Indicate that this is a remote inode
	    
        ret =  btrfs_create_common_mfs(mfs_dir, orig_dir, remote_dentry, inode, local_inode);
        //ret = btrfs_create(remote_idmap, mfs_dir, remote_dentry, mode, 1);
        //vfs_create(remote_idmap, mfs_dir, remote_dentry, mode, 1);
        //printk(KERN_INFO "btrfs_create_mfs: Remote inode is created with id %llu \n", inode->i_ino);
        done_path_create(&remote_path, remote_dentry);
        inode->associated_dir_inode = orig_dir; //Update the original directory inode for this remote inode 
        //printk(KERN_INFO "btrfs_create_mfs: Remote dentry is created\n");
        
        dentry->associated_dentry = remote_dentry;
        dentry->associated_path = &remote_path;
        remote_dentry->associated_dentry = dentry;

        //printk(KERN_INFO "btrfs_create_mfs: Shadow dentry is created\n");
        //printk(KERN_INFO "btrfs_create_mfs: Local inode is created with id %llu for remote inode %llu\n", local_inode->i_ino, inode->i_ino);
        //printk(KERN_INFO "btrfs_create_mfs: Did update associated_inode %llu of the local inode %llu \n", inode->i_ino, local_inode->i_ino);
        //goto out;
    }

//create_inode:
//        ret = btrfs_create(idmap, orig_dir, dentry, mode, excl);
//out:
        return ret;
}
		      ...
		      struct dentry *dentry)
{
    ...
    printk(KERN_INFO "btrfs_linked: Called...\n");
...
{
	struct inode *inode;
    printk(KERN_INFO "btrfs_mkdir: Called...\n");
...
static int btrfs_mkdir_mfs(struct mnt_idmap *idmap, struct inode *dir,
		       struct dentry *dentry, umode_t mode)
{
	struct inode *inode;
    //printk(KERN_INFO "btrfs_mkdir: Called with is_local_dir = %d, and dentry with remote = %d...\n", dir->is_local_dir, dentry->remote);
    struct btrfs_fs_info *fs_info = btrfs_sb(dir->i_sb);
    int fs_id = fs_info->id;
    printk(KERN_INFO "btrfs_mkdir_mfs did create a local directory...\n ");
	...
	inode->i_op = &btrfs_dir_inode_operations;
	inode->i_fop = &btrfs_dir_file_operations;
    //mfs: Update is_local_dir attribute
    inode->is_local_dir = 1;
    inode->has_remote = 2; //This is a local inode without a remote inode for it
    dentry->is_dir = 1;//This dentry if for directory
    return btrfs_create_common(dir, dentry, inode);
}
...
{
    printk(KERN_INFO "btrfs_readahead: Start to read from read_ahead...\n");
			 ...
			 u32 request_mask, unsigned int flags)
{
    ...
    //mfs: Checking
    //printk(KERN_INFO "btrfs_getattr: Called with inode of fs_id %d...\n", inode->fs_id);
...
static const struct inode_operations btrfs_dir_inode_operations = {
	...
	.lookup		= btrfs_lookup_mfs,
	.create		= btrfs_create_mfs,
	.unlink		= btrfs_unlink,
	.link		= btrfs_link,
	.mkdir		= btrfs_mkdir_mfs,
	.rmdir		= btrfs_rmdir,
