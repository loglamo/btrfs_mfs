...
//	printk(KERN_INFO "dat: device_name: %s, mode: %d, partition_order: %d\nbtrfs_scan_one_device_partition() done!\n", device_name, mode, partition_order);
	...
	if (error)
		goto error_fs_info;
//	printk(KERN_INFO "dat: btrfs_open_devices() done!\n");
...
//	printk(KERN_INFO "dat: sget() done!\n");
	if (s->s_root) {
...
static struct btrfs_fs_info *btrfs_mount_root_iter(struct file_system_type *fs_type,
		int flags, const char *device_name, void *data, int iter)
{
	struct block_device *bdev = NULL;
	struct super_block *s;
	struct btrfs_device *device = NULL;
    ...
    fs_info->id = iter;
    printk(KERN_INFO "btrfs_mount_root with fs_info id is %d\n", fs_info->id);
	fs_info->super_copy = kzalloc(BTRFS_SUPER_INFO_SIZE, GFP_KERNEL);
	fs_info->super_for_commit = kzalloc(BTRFS_SUPER_INFO_SIZE, GFP_KERNEL);
	if (!fs_info->super_copy || !fs_info->super_for_commit) {
		error = -ENOMEM;
...
// La: Get super_block from its s_root
/*
struct super_block *btrfs_sb_dentry(struct dentry **s_root) {
    return container_of(s_root, struct super_block, s_root);
}
*/

static struct dentry *btrfs_mount_root_mp(struct file_system_type *fs_type,
		int flags, const char *device_name, void *data) {
    int cpu_num = num_online_cpus();
    struct dentry *ret;
    struct dentry *temp;
    struct btrfs_fs_info *fs_info_temp;
    struct btrfs_fs_info *fs_info_ret;
    struct btrfs_fs_info *master;
    struct super_block *sb;

    printk(KERN_INFO"num online cpu is %d\n", cpu_num);
    for (int i = 0; i < cpu_num; i++) {
        fs_info_temp = btrfs_mount_root_iter(fs_type, flags, device_name, data, i);
        if (i == 0) {
            master = fs_info_temp; 
            fs_info_ret = fs_info_temp;
        }
         list_add_tail(&fs_info_temp->fs_info_list, &master->global_fs_info_list);
         master->fs_info_count++;
         printk(KERN_INFO "btrfs_mount_root: Updated fs_info_list of the master\n");
        
    }
    printk(KERN_INFO "btrfs_mount_root: id of global_fs_info is %d\n", fs_info_ret->id);
    printk(KERN_INFO "btrfs_mount_root: Updated fs_info_list of the master\n");


    if (!list_empty(&fs_info_ret->global_fs_info_list))
        printk(KERN_INFO "btrfs_mount_root: Done with move global list to the fs_info master with %d fs_info\n", fs_info_ret->fs_info_count);
    sb = fs_info_ret->sb;
