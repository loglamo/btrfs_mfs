...
static int btrfs_get_bdev_and_sb(const char *device_path, blk_mode_t flags, void *holder,
...
// dat: this function is called from btrfs_open_one_device_partition
static int btrfs_get_bdev_and_sb_partition(const char *device_path, blk_mode_t flags, void *holder,
		      int flush, struct block_device **bdev,
		      struct btrfs_super_block **disk_super, int partition_order)
{
	int ret;
	...
	*bdev = blkdev_get_by_path(device_path, flags, holder, NULL);
	...
	invalidate_bdev(*bdev);
	*disk_super = btrfs_read_dev_super_partition(*bdev, partition_order);
	if (IS_ERR(*disk_super)) {
		ret = PTR_ERR(*disk_super);
		blkdev_put(*bdev, holder);
		goto error;
...
// dat: this function call from btrfs_open_one_device_partition
static int btrfs_open_one_device_partition(struct btrfs_fs_devices *fs_devices,
			struct btrfs_device *device, blk_mode_t flags,
			void *holder, int partition_order)
{
	struct block_device *bdev;
	struct btrfs_super_block *disk_super;
	u64 devid;
	...
	ret = btrfs_get_bdev_and_sb_partition(device->name->str, flags, holder, 1,
				    &bdev, &disk_super, partition_order);
	if (ret)
		return ret;
...
// dat: this function call from btrfs_open_devices_partition when mounting the root fs
static int open_fs_devices_partition(struct btrfs_fs_devices *fs_devices,
				blk_mode_t flags, void *holder, int partition_order)
{
	struct btrfs_device *device;
	struct btrfs_device *latest_dev = NULL;
	struct btrfs_device *tmp_device;
	...
	list_for_each_entry_safe(device, tmp_device, &fs_devices->devices,
		...
		ret = btrfs_open_one_device_partition(fs_devices, device, flags, holder, partition_order);
		if (ret == 0 &&
		    (!latest_dev || device->generation > latest_dev->generation)) {
			latest_dev = device;
		} else if (ret == -ENODATA) {
...
// dat: this function is called by btrfs_mount_root when mounting the root fs
int btrfs_open_devices_partition(struct btrfs_fs_devices *fs_devices,
		       blk_mode_t flags, void *holder, int partition_order)
{
	int ret;
	...
	if (fs_devices->opened) {
		...
		list_sort(NULL, &fs_devices->devices, devid_cmp);
		ret = open_fs_devices_partition(fs_devices, flags, holder, partition_order);
	}
...
// dat: this function is called by btrfs_mount_root when mounting the root fs
struct btrfs_device *btrfs_scan_one_device_partition(const char *path, blk_mode_t flags, int partition_order)
{
	struct btrfs_super_block *disk_super;
	bool new_device_added = false;
	struct btrfs_device *device = NULL;
	...
	if (IS_ERR(bdev))
		return ERR_CAST(bdev);
	u64 sb_offset = partition_order * BTRFS_SUPER_INFO_SIZE;
	bytenr_orig = btrfs_sb_offset_partition(0, sb_offset);
	ret = btrfs_sb_log_location_bdev_partition(bdev, 0, READ, &bytenr, sb_offset);
	if (ret) {
		device = ERR_PTR(ret);
		goto error_bdev_put;
	}
    ...
    /*    
    printk(KERN_INFO "scan_one_device: total bytes is %llu", disk_super->total_bytes);
    printk(KERN_INFO "scan_one_device: sectorsize is %llu", disk_super->sectorsize);
    printk(KERN_INFO "scan_one_device: nodesize is %llu", disk_super->nodesize);
    */
    ...
    /*
    printk(KERN_INFO "scan_one_device: start sector is %llu", bdev->bd_start_sect);
    printk(KERN_INFO "scan_one_device: nrsectors is %llu", bdev->bd_nr_sectors);
    printk(KERN_INFO "scan_one_device: total bytes is %llu", disk_super->total_bytes);
    printk(KERN_INFO "scan_one_device: sectorsize is %llu", disk_super->sectorsize);
    printk(KERN_INFO "scan_one_device: nodesize is %llu", disk_super->nodesize);
	*/
...
// dat: this function is call from read_one_dev: add initial for partition_start and partition_end
static void fill_device_from_item_partition(struct extent_buffer *leaf,
				 struct btrfs_dev_item *dev_item,
				 struct btrfs_device *device, int partition_order)
{
	unsigned long ptr;

	device->devid = btrfs_device_id(leaf, dev_item);
	device->disk_total_bytes = btrfs_device_total_bytes(leaf, dev_item);
	// dat: add initial for partition_start and partition_end
	device->partition_start = device->disk_total_bytes*partition_order;
	device->partition_end = device->disk_total_bytes*(partition_order+1);

	device->total_bytes = device->disk_total_bytes;
    //printk(KERN_INFO "fill_device_from_partition: Total bytes of disk is %llu\n", device->total_bytes);
	device->commit_total_bytes = device->disk_total_bytes;
	device->bytes_used = btrfs_device_bytes_used(leaf, dev_item);
	device->commit_bytes_used = device->bytes_used;
	device->type = btrfs_device_type(leaf, dev_item);
...
// dat: this function is called by btrfs_read_chunk_tree
static int read_one_dev_partition(struct extent_buffer *leaf,
			struct btrfs_dev_item *dev_item, int partition_order)
{
	BTRFS_DEV_LOOKUP_ARGS(args);
	struct btrfs_fs_info *fs_info = leaf->fs_info;
	struct btrfs_fs_devices *fs_devices = fs_info->fs_devices;
	...
	fill_device_from_item_partition(leaf, dev_item, device, partition_order);
	if (device->bdev) {
		u64 max_total_bytes = bdev_nr_bytes(device->bdev);
...
// dat: modify this fucntion to read the chunk tree with partition
int btrfs_read_chunk_tree_partition(struct btrfs_fs_info *fs_info, int partition_order)
{
	struct btrfs_root *root = fs_info->chunk_root;
	struct btrfs_path *path;
	struct extent_buffer *leaf;
	...
	btrfs_for_each_slot(root, &key, &found_key, path, iter_ret) {
		...
		if (found_key.type == BTRFS_DEV_ITEM_KEY) {
			...
			dev_item = btrfs_item_ptr(leaf, slot,
						  struct btrfs_dev_item);
			ret = read_one_dev_partition(leaf, dev_item, partition_order);
			if (ret)
				goto error;
			total_dev++;
		} else if (found_key.type == BTRFS_CHUNK_ITEM_KEY) {
...
"dev extent physical offset %llu on devid %llu length doesn't match chunk %llu, have %llu expect %llu",
	...
	// dat: we should check with device->partition_end
	// if (physical_offset + physical_len > dev->disk_total_bytes) {
	if (physical_offset + physical_len > dev->partition_end) {
		btrfs_err(fs_info,
...
"dev extent devid %llu physical offset %llu overlap with previous dev extent end %llu",
		...
		// printk("devid %llu physical offset %llu len %llu chunk offset %llu\n",
		    //    devid, physical_offset, physical_len, chunk_offset);
