			...
			struct dentry *dentry, umode_t mode, bool excl)
{
    ...
    //printk(KERN_INFO "btrfs_create_mfs: File name is %s \n", name);
    ...
    if (remote_device == local_device) { // Case: Assigned FS is the local FS
	    ...
	    inode->i_op = &btrfs_file_inode_operations;
	    inode->i_mapping->a_ops = &btrfs_aops;
	    ret =  btrfs_create_common(mfs_dir, remote_dentry, inode);
        ...
        //printk(KERN_INFO "btrfs_create_mfs: Local inode is created with id %llu \n", local_inode->i_ino);
