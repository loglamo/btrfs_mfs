...
int btrfs_new_inode_prepare_mp(struct btrfs_new_inode_args *args,
			    unsigned int *trans_num_items)
{
	struct inode *dir = args->dir;
	struct inode *inode = args->inode;
...
int btrfs_create_new_inode_mp(struct btrfs_trans_handle *trans,
			   struct btrfs_new_inode_args *args, struct btrfs_fs_info *fs_info)
{
	struct inode *dir = args->dir;
	struct inode *inode = args->inode;
	const struct fscrypt_str *name = args->orphan ? NULL : &args->fname.disk_name;
...
static int btrfs_create_common_mp(struct inode *dir, struct dentry *dentry,
			       struct inode *inode, struct btrfs_fs_info *allocated_fs_info)
{   /*
	struct btrfs_fs_info *fs_info = btrfs_sb(dir->i_sb);
    struct btrfs_fs_info *allocated_fs_info;
    struct btrfs_fs_info *fs_info_temp;
    */
    printk(KERN_INFO "btrfs_create_common_mp: allocated fs_info is %d\n", allocated_fs_info->id);

    /*
    struct list_head global_fs_info_list = fs_info->global_fs_info_list;
    if (!list_empty(&global_fs_info_list)) {
        int cpu_id = get_cpu();
        printk(KERN_INFO "btrfs_create_common_mp: work on CPU %d\n", cpu_id);
        struct list_head *pos;
        list_for_each(pos, &global_fs_info_list) {
            fs_info_temp = list_entry(pos, struct btrfs_fs_info, fs_info_list);
            if (fs_info_temp->id == cpu_id) {
                allocated_fs_info = fs_info_temp;
                break;
            }
        }
    } else {
        allocated_fs_info = fs_info;
    }
     */

	//struct btrfs_root *root = BTRFS_I(dir)->root;
	struct btrfs_root *root = allocated_fs_info->tree_root;
	struct btrfs_new_inode_args new_inode_args = {
		.dir = dir,
		.dentry = dentry,
		.inode = inode,
	...
	err = btrfs_create_new_inode_mp(trans, &new_inode_args, allocated_fs_info);
	if (!err)
		d_instantiate_new(dentry, inode);

	btrfs_end_transaction(trans);
	btrfs_btree_balance_dirty(allocated_fs_info);
out_new_inode_args:
	btrfs_new_inode_args_destroy(&new_inode_args);
out_inode:
	if (err)
...
// Assign new sb to the new inode
static int btrfs_create_mp(struct mnt_idmap *idmap, struct inode *dir,
			struct dentry *dentry, umode_t mode, bool excl)
{
	struct inode *inode;
    
    // Get the master fs_info and super_block
    struct btrfs_fs_info *fs_info = btrfs_sb(dir->i_sb);
    struct btrfs_fs_info *allocated_fs_info;
    struct btrfs_fs_info *fs_info_temp;
    struct list_head global_fs_info_list = fs_info->global_fs_info_list;
    ...
    if (!list_empty(&global_fs_info_list)) {
        int cpu_id = get_cpu();
        printk(KERN_INFO "btrfs_create: Get CPUid\n");
        if (cpu_id != 0) {
           struct list_head *pos;
           list_for_each(pos, &global_fs_info_list) {
              fs_info_temp = list_entry(pos, struct btrfs_fs_info, fs_info_list);
              if (fs_info_temp->id == cpu_id) {
                allocated_fs_info = fs_info_temp;
                printk(KERN_INFO "fs_info %d is allocated\n", fs_info_temp->id);
                break;
             }
         }
        } else 
    ...
    printk(KERN_INFO "btrfs_create: fs_info %d is assigned to the new inode\n", allocated_fs_info->id);
    struct super_block *allocated_sb = allocated_fs_info->sb;

    // New inode at allocated super block
	//inode = new_inode(dir->i_sb);
	inode = new_inode(allocated_sb);
	if (!inode)
		return -ENOMEM;
	inode_init_owner(idmap, inode, dir, mode);
	inode->i_fop = &btrfs_file_operations;
	inode->i_op = &btrfs_file_inode_operations;
	inode->i_mapping->a_ops = &btrfs_aops;
	return btrfs_create_common_mp(dir, dentry, inode, allocated_fs_info);
}
